'''
This file allows visualizing the results from the simulations.
...
if __name__ == '__main__':
    ...
    for file in os.listdir(args.input):
        with open(os.path.join(args.input, file), 'r') as file:
            ...
            if content['algorithm'] == 'fedprox':
                algorithm = 'fedprox (alpha = {})'.format(content['alpha'])
            elif content['algorithm'] == 'ours':
                algorithm = 'ours (beta = {})'.format(content['beta'])
            ...
            table = [
                {
                    ...
                    'max_gradient_norm': 'G = {:4}'.format(content['max_gradient_norm']),
    ...
    frame = pd.DataFrame(frame).sort_values(by = [ 'scheme', 'dataset', 'balancedness', 'convexity', 'max_gradient_norm', 'step_size', 'algorithm', 'round', 'metric' ])
    ...
    fgrid.set_titles(col_template = '{col_name}', row_template = '')
